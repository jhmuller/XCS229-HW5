...
def main(is_semi_supervised, trial_num):
    ...
    # *** START CODE HERE ***
    w = np.ones((n, K)) / K
    group = np.random.choice(range(K), n, replace=True)
    mu = np.array([x[group == i].mean(axis=0) for i in range(K)])
    sigma = np.array([np.cov(x[group == i],rowvar=False, aweights=w[group==i,i]) for i in range(K)])
    phi = np.ones(K) / K
    print("")
    # *** END CODE HERE ***
...
def run_em(x, w, phi, mu, sigma, max_iter=1000):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        prev_ll = ll
        ll = 0
        def guassian_prob(x, mu, sigma):
            if sigma.shape[0] == 1:
                num =  np.exp(-0.5 * np.sum((x - mu) ** 2 / sigma))
                denom = np.sqrt(2 * np.pi * sigma)
                res = num / denom
            else:
                res = np.zeros(x.shape[0])
                denom = np.sqrt(np.linalg.det(sigma)) * (2 * np.pi) ** (x.shape[1] / 2)  
                siginv = np.linalg.inv(sigma)              
                for i in range(x.shape[0]):
                    z = x[i,:]
                    num =  np.exp(-0.5 * np.sum((z - mu) @ siginv @ (z - mu).T))
                    p1 = np.matmul(np.matmul((z - mu), siginv), (z-mu).T)
                    num = np.exp(-0.5 * p1)                
                    res[i] = num / denom
            return res
        ...
        # E step
        for j in range(K):
            probs = guassian_prob(x, mu[j], sigma[j])
            w[:, j] = phi[j] * probs            
        ...
        for j in range(K):
            # M step
            try:
                xx = np.sum(w[:,j], axis=0) / x.shape[0]
                phi[j] = np.sum(w[:,j], axis=0) / x.shape[0]
        ...
        try:
            ll =  np.sum([np.log(np.sum([np.exp(-0.5 * np.sum((x[i, :] - mu[j]) ** 2 / sigma[j])) 
                        for j in range(K)])) for i in range(x.shape[0])])
        except Exception as e:
                import sys
                exc_type, exc_obj, exc_tb = sys.exc_info()
                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                ...
                break   
        if it % printfreq == 0:
            print(f" it {it} ll {ll}, prev_ll {prev_ll}")
    print("done ll {ll} prev_ll {prev_ll}")
        # *** END CODE HERE ***
...
def run_semi_supervised_em(x, x_tilde, z_tilde, w, phi, mu, sigma, max_iter=1000):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        try:
            ll =  np.sum([np.log(np.sum([np.exp(-0.5 * np.sum((x[i, :] - mu[j]) ** 2 / sigma[j])) for j in range(K)])) for i in range(x.shape[0])])
        except Exception as e:
                import sys
                exc_type, exc_obj, exc_tb = sys.exc_info()
                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
